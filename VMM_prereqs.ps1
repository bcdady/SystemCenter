#===============================================================================
# NAME      : VMM_prereqs.ps1
# LANGUAGE  : Windows PowerShell
# AUTHOR    : Bryan Dady
# DATE      : 02/16/2012
# COMMENT   : PowerShell script to check, enable and configure basic System Center 
# 			  Virtual Machine Manager 2012 pre-requisites on Windows Server 2008 R2
#===============================================================================
...
$pathToMSI = join-path $installRoot "Prerequisites\WAIK\wAIKAMD64.msi"; # set this to a fully qualified path if the MSI is not in the same directory as this script
if (test-path $pathToMSI) {
	write-host "`nInstalling MSI package $pathToMSI (Windows Automated Installation Kit)";
...
#===============================================================================
$pathToMSI = join-path $installRoot "Prerequisites\SQLNC\sqlncli.msi"; # set this to a fully qualified path if the MSI is not in the same directory as this script
if (test-path $pathToMSI) {
...
#===============================================================================
$pathToMSI = join-path $installRoot "Prerequisites\SQLCLU\SqlCmdLnUtils.msi"; # set this to a fully qualified path if the MSI is not in the same directory as this script
if (test-path $pathToMSI) {
...
if (test-path $pathToSetup) {
    ...
    write-warning Unable to locate VMM setup VMM files
    break;
}
# call checkProcess function for each MSI
